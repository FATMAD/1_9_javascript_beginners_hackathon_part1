...
function logMessage(username, timestamp, url) {
	...
	var message= username +" accessed the site " + url+ " at " + timestamp +".";
	return message;
}
...
function substringBetweenMatches(str, subStr) {
	...
	var n= str.lastIndexOf(subStr) ;
     var m = str.indexOf(subStr)+subStr.length ;
...
function both_ends(count) {
	if (count.lengh <2){
     ...
     return (count[0] + count[1] + count[count.length-2] + count[count.length-1]);
...
function not_bad(str) {
		...
		var m = str.indexOf('not');
        var k= str.indexOf('bad');

        if (m == -1 || k == -1 || k < m) 
        	return str;

        else
  return str.slice(0, m) + 'good' + str.slice(k + 3);
